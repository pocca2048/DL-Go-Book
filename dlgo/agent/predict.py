...
class DeepLearningAgent(Agent):
    def __init__(self, model, encoder: encoders.Encoder) -> None:
        super().__init__()
        self.model = model
    ...
    def select_move(self, game_state: goboard.GameState):
        ...
        move_probs = move_probs ** 3 # 차이를 크게 하기
        eps = 1e-6
        move_probs = np.clip(move_probs, eps, 1-eps) # 0이나 1에 가까운 수는 제거
        move_probs = move_probs / np.sum(move_probs) # 다시 normalize
