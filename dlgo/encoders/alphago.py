...
Stone colour            3               돌 색. Player stone / opponent stone / empty
Ones                    1               A constant plane filled with 1 왜??
Zeros                   1               A constant plane filled with 0 왜??
Sensibleness            1               Whether a move is legal and does not fill its own eyes
Turns since             8               어떤 수가 놓인 후 몇 수가 지났는지를 센다.
Liberties               8               활로의 개수. 1~8개 까지를 세므로 8개 plane.
Liberties after move    8               Number of liberties after this move is played
Capture size            8               How many opponent stones would be captured
Self-atari size         8               자충수. 이 수를 두면 자신의 돌이 몇 개나 단수 상태에 빠지고, 다음 동작에서 잡힐지.
Ladder capture          1               축잡기. Whether a move at this point is a successful ladder capture
Ladder escape           1               축에서 벗어나기. Whether a move at this point is a successful ladder escape
마지막 하나는 현재 돌 색으로 Value Network에서만 사용한다.
"""
    ...
    def encode(self, game_state):
        ...
        for r in range(self.board_height):
            for c in range(self.board_width):
                ...
                if ages > 0:
                    print(ages)
                    board_tensor[offset("turns_since") + int(ages)][r][c] = 1 # changed ages -> int(ages)
